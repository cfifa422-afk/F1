...
async def _send_vote_reminder(user: discord.User):
    """DM a player a one-time (per 3-day window) top.gg vote reminder after catching a card."""
    try:
        vote_url = "https://top.gg/bot/1228631433501999124/vote"
        embed = discord.Embed(
            title="🗳️  Enjoying the bot? Vote for us!",
            description=(
                "Thanks for catching a card! If you're having fun, **support us on Top.gg** — it only takes a second.\n\n"
                "**Vote rewards:**\n"
                "🪙 **200 Race Credits**\n"
                "🏁 **1 extra Career Match**\n"
                "📦 **1 Base Pack**\n\n"
                "Use `/vote` in the bot's channel to claim your rewards after voting!"
            ),
            color=0xFF5F5F,
        )
        embed.set_footer(text="You'll only be reminded once every 3 days.")
...
# ── Paste your GIF/image URLs here. Leave a key as "" to skip it silently. ──
EVENT_GIF_URLS: Dict[str, str] = {
    "pit_window":       "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTRycHV4ajJjOGY4c3owYTllMjIyODM2dnJ6ZjhxOTFmb2Q2M3V5ZCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/MovqJSMROh1gA/giphy.gif",   # Turn 3 — pit window opens
    "drs_attack":       "https://c.tenor.com/f5kD8E5xCggAAAAd/tenor.gif",   # Turn 6 — DRS attack zone
    "late_strategy":    "",   # Turn 9 — late race push
    "final_lap":        "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExaHNnOXB5bnUxcGdmeGdvdTF0MXh0enJtajJueXR0YTlqaWp6YTd4ZSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/ScjojCIikEkaXJHQlv/giphy.gif",   # Turn 11 — white flag lap
    "rain_call":        "https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExNXkzNG9qcmw0NWU3dmFlYXM2a2c4M290NXdwY3Zvd20xZzA4bzVkbCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/F0ygrGeQ5DdjifEgtz/giphy.gif",   # Rain scenario (box for wets?)
    "reaction":         "",   # Reaction challenge fires in channel
    "safety_car":       "https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExcTB6em44NjJ0aXNxbzRwYm9mYzNrcHB1cWxjMjZuZ2h4eG9wM3pnMiZlcD12MV9naWZzX3NlYXJjaCZjdD1n/LKqJo3ObrpCQHFuxHp/200.webp",   # Safety car event
    "wrong_direction":  "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExNml2NGxrcjkwMjY3NWFrdGg0ODc0ZG52Z2JzbnQyNGI2M3c0dHRxbiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/ys5paotHEo6WY/giphy.gif",   # Player clicks the wrong arrow
    "correct_fast":     "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExZmVqaXRnZ3B6MXR6YTQydDZzbWszN21jbDNoaTl4Nmdsa3Jla2Y2aiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/F3JhKaucb9QqSF7bSS/giphy.gif",   # Player nails the reaction
    "gap_opening":      "https://tenor.com/view/lewis-hamilton-max-verstappen-formula-one-silverstone-british-gp-gif-22400521",   # Gap grows past 3 s
    "p1_cooking":       "https://c.tenor.com/Za0xjZTF3voAAAAC/tenor.gif",   # Leader pulls 6 s+ ahead
    "pit_stop_done":    "",   # Player chose pit stop
}


async def _send_event_gif(channel, gif_key: str, delete_after: Optional[float] = None, content: Optional[str] = None):
    """Send a GIF URL embed to the channel with an optional ping/text. Silently skips if URL is empty."""
    url = EVENT_GIF_URLS.get(gif_key, "")
    if url:
        try:
...
async def _send_dm_gif(user: discord.Member, gif_key: str):
    """Send a GIF to a player's DMs as a plain URL (no embed wrapper, stays permanently)."""
    url = EVENT_GIF_URLS.get(gif_key, "")
    if url:
        try:
            await user.send(url)
...
class CatchModal(discord.ui.Modal, title="Catch the Card!"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if entered == correct_name or (correct_code and entered == correct_code):
            ...
            # Vote reminder — once per 3 days per player
            if db.should_send_vote_reminder(player_id):
                db.set_vote_reminder_sent(player_id)
                asyncio.create_task(_send_vote_reminder(interaction.user))
        else:
            await interaction.response.send_message(
...
async def daily_promo_dm():
    ...
    for pid in player_ids:
        ...
        try:
            ...
            await user.send(A major update! 
Career mode has been added🔥
Use /career to start your f1 career and /career_matches to play your f1 matches 

Do not forget to vote us on top.gg for a free pack! And an extra career game match. (/Vote)

The rewards are very good, grind that matches and do your best 🔥

Notice: check dms for reaction challenge after starting a career match and when it does not spawn go back to the main channel to choose an option. 
...
class DMScenarioView(discord.ui.View):
    """Scenario choice view sent privately to one player via DM."""

    def __init__(self, player: discord.Member, scenario: Dict, done_event: asyncio.Event):
    ...
    async def _handle(self, interaction: discord.Interaction, value: str, label: str):
        ...
        is_pit = value == "pit_stop"
        if is_pit:
            await interaction.response.edit_message(
                content=f"⛽ **Pit stop locked in!** Heading to the pits…\nWaiting for your opponent to decide…",
                view=None,
            )
            await _send_dm_gif(self.player, "pit_stop_done")
        else:
            await interaction.response.edit_message(
                content=f"✅ **{label}** locked in! Waiting for your opponent…",
                view=None,
            )

    async def on_timeout(self):
        if self.choice is None:
            self.choice = "same_speed"
            self.label  = "Auto (timed out)"
        self.done.set()


class DMReactionView(discord.ui.View):
    """Reaction challenge sent privately to one player via DM."""
    ...
    async def _handle(self, interaction: discord.Interaction, clicked: str):
        ...
        if correct:
            ...
            await _send_dm_gif(self.player, "correct_fast")
        else:
            await interaction.response.edit_message(
            ...
            await _send_dm_gif(self.player, "wrong_direction")
...
async def _run_dm_scenario(
    p1_user: discord.Member,
    p2_user: discord.Member,
    scenario: Dict,
    race: RaceState,
    channel_msg: discord.Message,
    gif_url: str,
    timeout: float = 32,
) -> tuple:
    """DM both players their scenario choices. Returns (p1_choice, p2_choice, p1_label, p2_label)."""
    p1_done = asyncio.Event()
    p2_done = asyncio.Event()

    sv1 = DMScenarioView(p1_user, scenario, p1_done)
    sv2 = DMScenarioView(p2_user, scenario, p2_done)

    is_rain_pit = scenario.get("id") == "rain_call"

    def _build_dm_embed(player: discord.Member, opponent: discord.Member, is_p1: bool) -> discord.Embed:
        fuel   = race.p1_fuel  if is_p1 else race.p2_fuel
        wear   = race.p1_tire_wear if is_p1 else race.p2_tire_wear
        t_type = race.p1_tire_type if is_p1 else race.p2_tire_type
        gap    = race.gap
        ...
        embed.add_field(
            name="📊  Your Status",
            value=(
                f"⛽ Fuel: **{fuel:.0f}%**  ·  {_tire_str(wear, t_type)}\n"
                f"🏎️ Lap **{race.lap}/{race.total_laps}**"
            ),
            inline=False,
        )
        if abs(gap) < 0.3:
            gap_str = "⚡ **Side by side!**"
        elif (is_p1 and gap < 0) or (not is_p1 and gap > 0):
            gap_str = f"✅ **You lead** by **{abs(gap):.2f}s**"
        else:
            gap_str = f"⚠️ **{opponent.display_name}** leads by **{abs(gap):.2f}s**"
        embed.add_field(name="🏆  Gap", value=gap_str, inline=True)
        embed.set_footer(text="⏱️  You have 30 seconds — choose wisely!")
        return embed
    ...
    try:
        dm_embed_p1 = _build_dm_embed(p1_user, p2_user, is_p1=True)
        if is_rain_pit:
            await _send_dm_gif(p1_user, "rain_pit")
        else:
            await _send_dm_gif(p1_user, "scenario_dm")
        await p1_user.send(embed=dm_embed_p1, view=sv1)
        p1_sent = True
    except discord.Forbidden:
        p1_done.set()
    except Exception:
    ...
    try:
        dm_embed_p2 = _build_dm_embed(p2_user, p1_user, is_p1=False)
        if is_rain_pit:
            await _send_dm_gif(p2_user, "rain_pit")
        else:
            await _send_dm_gif(p2_user, "scenario_dm")
        await p2_user.send(embed=dm_embed_p2, view=sv2)
        p2_sent = True
    except discord.Forbidden:
        p2_done.set()
    except Exception:
    ...
    try:
        await asyncio.wait_for(
            asyncio.gather(
                asyncio.create_task(p1_done.wait()),
                asyncio.create_task(p2_done.wait()),
            ),
            timeout=timeout,
        )
    except asyncio.TimeoutError:
        sv1.on_timeout if not p1_done.is_set() else None
        sv2.on_timeout if not p2_done.is_set() else None
        if not p1_done.is_set():
            sv1.choice = sv1.choice or "same_speed"
            sv1.label  = sv1.label  or "Auto (timed out)"
        if not p2_done.is_set():
            sv2.choice = sv2.choice or "same_speed"
            sv2.label  = sv2.label  or "Auto (timed out)"

    return (
        sv1.choice or "same_speed",
        sv2.choice or "same_speed",
        sv1.label  or "Auto (timed out)",
        sv2.label  or "Auto (timed out)",
    )
...
async def run_auto_race(
    ...
    """Fully auto-simulated race — all menus in channel; DM pings notify players before decisions."""
    AUTO_DELAY    = 2.8
    SCENARIO_WAIT = 30
    commentary_log: List[str] = []
    gap_gif_ref   = [0]   # tracks which gap-GIF tier we last fired (3 or 6)

    def _next_scenario(current_turn: int) -> Optional[int]:
        return next((t for t in SCENARIO_TURNS if t > current_turn), None)

    async def _dm_ping(text: str):
        """Silently DM both players a heads-up. Never blocks the race."""
    ...
    async def _maybe_send_gap_gif(gap: float):
        """Fire contextual GIF in channel when gap crosses 3 s or 6 s thresholds."""
        abs_gap = abs(gap)
        prev    = gap_gif_ref[0]
        leader  = p1_user if gap < 0 else p2_user
        if abs_gap >= 6.0 and prev < 6:
            await _send_event_gif(channel, "p1_cooking",
                content=f"🔥 {leader.mention} is cooking and getting faster — nobody can touch them right now!")
            gap_gif_ref[0] = 6
        elif abs_gap >= 3.0 and prev < 3:
            await _send_event_gif(channel, "gap_opening",
                content=f"📡 {leader.mention} is pulling away! The gap is opening up!")
            gap_gif_ref[0] = 3

    # ── One-time DM at race start ──
    for u in (p1_user, p2_user):
        try:
            await u.send(
                "🏁 **Race is starting!**\n"
                "All your inputs (reaction challenges & strategy calls) will come here in your DMs — "
                "keep this chat open! 👀"
            )
        except Exception:
            pass
    ...
    try:
        for turn_num in range(1, race.max_turns + 1):
            ...
            if scenario:
                # ── SCENARIO PAUSE — buttons in channel, DM ping first ──
                scenario_title = scenario.get("title", "Race Decision")
                is_pit    = any(opt.get("value") == "pit_stop" for opt in scenario.get("options", []))
                is_rain   = scenario.get("id") == "rain_call"

                # 1. DM ping so players don't miss it
                pit_dm   = "  ⛽ Pit stop is available!" if is_pit else ""
                rain_dm  = "  🌧️ Rain is here — box for wets or gamble?" if is_rain else ""
                await _dm_ping(
                    f"📲 **Race decision time!** — **{scenario_title}**\n"
                    f"Head to the race channel to make your call!{pit_dm}{rain_dm}"
                )

                # 2. Scenario GIF in channel
                await _send_event_gif(channel, scenario.get("id", ""),
                    content=f"⚠️ {p1_user.mention} {p2_user.mention} — **{scenario_title}!** Check your DMs to make your call!")

                # 3. Pit-stop GIF in channel if applicable
                if is_pit:
                    await _send_event_gif(channel, "pit_stop_done",
                        content=f"🔧 {p1_user.mention} {p2_user.mention} — The pit window is OPEN!")

                # 4. DM both players their scenario choices simultaneously
                p1_choice, p2_choice, p1_label, p2_label = await _run_dm_scenario(
                    p1_user, p2_user, scenario, race, message, gif_url
                )

                # Show both choices locked in the channel embed briefly
                try:
                    await message.edit(
                        embed=build_scenario_embed(
            ...
            try:
                new_lines = commentary_engine.generate_turn_commentary(
                    turn=race.turn, lap=race.lap, total_laps=race.total_laps,
                    p1_name=p1_user.display_name, p2_name=p2_user.display_name,
                    p1_choice=p1_choice, p2_choice=p2_choice,
                    gap=race.gap, event=event,
                    p1_fuel=race.p1_fuel, p2_fuel=race.p2_fuel,
                    p1_tire_wear=race.p1_tire_wear, p2_tire_wear=race.p2_tire_wear,
                    p1_tire_type=race.p1_tire_type, p2_tire_type=race.p2_tire_type,
                    p1_leading=(race.p1_position == 1), weather=race.weather,
                )
            ...
            # Safety car GIF
            if event and "Safety" in event:
                await _send_event_gif(channel, "safety_car",
                    content=f"🚗 {p1_user.mention} {p2_user.mention} — Safety Car is out!")
            ...
            # Dynamic gap GIF
            await _maybe_send_gap_gif(race.gap)

            # ── REACTION CHALLENGE — embed + buttons in DMs, GIF separate ──
            if turn_num in REACTION_TURNS and channel:
                ...
                def _reaction_embed() -> discord.Embed:
                    """Clean embed — no GIF inside, GIF sent separately before this."""
                    e = discord.Embed(
                        title="⚡  REACTION CHALLENGE!",
                        description=(
                            f"Hit  **{dir_label}**  as fast as you can!\n\n"
                            f"4 directions · only YOU can click this\n"
                            f"❌ Wrong = **+2.0s penalty**  |  ⏱️ Miss = **+0.3s penalty**"
                        ),
                        color=0xF1C40F,
                    )
                ...
                # ── Send embed + GIF to BOTH players simultaneously ──
                rv1 = DMReactionView(direction, p1_user)
                rv2 = DMReactionView(direction, p2_user)

                async def _deliver_p1():
                    try:
                        await _send_dm_gif(p1_user, "reaction")
                        await p1_user.send(embed=_reaction_embed(), view=rv1)
                    except Exception:
                        rv1.done.set()

                async def _deliver_p2():
                    try:
                        await _send_dm_gif(p2_user, "reaction")
                        await p2_user.send(embed=_reaction_embed(), view=rv2)
                    except Exception:
                        rv2.done.set()

                await asyncio.gather(_deliver_p1(), _deliver_p2())

                # ── Wait for both responses (parallel, max 4.5 s each) ──
                async def _wait_p1():
                    try:
                        await asyncio.wait_for(rv1.done.wait(), timeout=4.5)
                    except asyncio.TimeoutError:
                        rv1.stop()

                async def _wait_p2():
                    try:
                        await asyncio.wait_for(rv2.done.wait(), timeout=4.5)
                    except asyncio.TimeoutError:
                        rv2.stop()

                await asyncio.gather(_wait_p1(), _wait_p2())

                # ── Evaluate results ──
                p1_result = rv1.result
                ...
                for i, (user, elapsed, side) in enumerate(correct_clicks):
                    if i == 0:
                        race.gap -= 0.5 if side == "p1" else -0.5
                        any_correct = True
                    medal = "🥇" if i == 0 else "🥈"
                ...
                # Contextual GIFs in channel with ping (permanent)
                if any_wrong:
                    wrong_users = " ".join(
                        u.mention for u, res in ((p1_user, p1_result), (p2_user, p2_result))
                        if res and not res[0]
                    )
                    await _send_event_gif(channel, "wrong_direction",
                        content=f"❌ {wrong_users} went the wrong direction — penalty incoming! 💀")
                elif any_correct:
                    fastest = correct_clicks[0][0] if correct_clicks else None
                    ping = fastest.mention if fastest else f"{p1_user.mention} {p2_user.mention}"
                    await _send_event_gif(channel, "correct_fast",
                        content=f"🚀 {ping} nailed the reaction and is getting faster!")
...
async def finish_auto_race(
    ...
    # Edit the live race message
    try:
        await message.edit(embed=embed, view=None)
    except Exception:
        pass

    # Also post a standalone result embed in channel (always visible)
    if channel:
        try:
...
@commands.is_owner()
async def broadcast_dms(ctx, *, message: str):
    """
    !dms <message>
    Sends a DM from the bot to every registered player.
    Owner-only.
    """
    player_ids = db.get_all_player_ids()
    sent = 0
    failed = 0
    status_msg = await ctx.send(f"📨 Sending to **{len(player_ids)}** players…")
    ...
    for pid in player_ids:
        try:
            ...
            embed.set_author(name="📢  Message from F1 Bot", icon_url=bot.user.display_avatar.url)
            embed.set_footer(text="F1 Card Racing Bot")
            await user.send(embed=embed)
            sent += 1
        except Exception:
            failed += 1

    await status_msg.edit(content=f"✅ Sent to **{sent}** players. Failed: **{failed}**.")


# ==================== VOTE SYSTEM ====================

class VoteView(discord.ui.View):
    """Buttons for the /vote command: link to top.gg + claim rewards."""
    ...
    def __init__(self, player_id: str, can_claim: bool):
        ...
        claim_btn = discord.ui.Button(
            label="Claim Rewards" if can_claim else "Already Claimed (12h cooldown)",
    ...
    async def _claim_callback(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.player_id:
            await interaction.response.send_message("This isn't your vote menu!", ephemeral=True)
            return
        if not db.can_claim_vote(self.player_id):
            await interaction.response.send_message(
                "⏳ You already claimed recently. Vote rewards refresh every **12 hours**!", ephemeral=True
            )
            return

        # Give rewards
        db.add_coins(self.player_id, 200)
        db.add_vote_bonus_match(self.player_id)
        pack_cards = card_module.generate_pack("daily")
        card_names = []
        for c in pack_cards:
            db.add_card_to_player(self.player_id, c, c["type"])
            emoji = card_module.RARITY_EMOJIS.get(c["rarity"], "")
            label = f"{c['name']} ({c['code']})" if c.get("code") else c["name"]
            card_names.append(f"{emoji} {label} ({c['rarity'].title()})")
        db.set_vote_claimed(self.player_id)

        bonus = db.get_vote_bonus_matches(self.player_id)
        embed = discord.Embed(
            title="🎉  Vote Rewards Claimed!",
            description=(
                "Thank you for voting! Here's what you received:\n\n"
                f"🪙 **+200 Race Credits**\n"
                f"🏁 **+1 Career Match** *(you now have {bonus} bonus {'match' if bonus == 1 else 'matches'})*\n"
                f"📦 **Base Pack opened:**\n" + "\n".join(f"  • {n}" for n in card_names)
            ),
            color=0x00D060,
        )
        embed.set_footer(text="Come back in 12 hours to vote again!")
...
class CareerVoteView(discord.ui.View):
    """Shown when career daily limit is reached — use a bonus match or vote to get one."""

    def __init__(self, player_id: str, has_bonus: bool, cooldown_msg: str):
        super().__init__(timeout=120)
        self.player_id = player_id
    ...
    async def _use_bonus_callback(self, interaction: discord.Interaction):
        ...
        if not db.use_vote_bonus_match(self.player_id):
            await interaction.response.send_message("❌ No bonus matches left. Vote on Top.gg to earn one!", ephemeral=True)
            return
        self.used_bonus = True
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(
            content="✅ Bonus match used! Run `/career_match` again to play your extra race.",
            embed=None,
            view=self,
        )
        self.stop()
...
async def career_match_slash(interaction: discord.Interaction):
    ...
    if not ok:
        if reason == "season_done":
            ...
            embed.set_footer(text="Use /vote to claim voting rewards including bonus career matches.")
            view = CareerVoteView(player_id, has_bonus=bonus > 0, cooldown_msg=cooldown_str)
...
@bot.tree.command(name="vote", description="Vote for the bot on Top.gg and claim your rewards!")
async def vote_slash(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="🗳️  Vote for Us on Top.gg!",
        description=(
            "Support the bot by voting on **Top.gg**! After you vote, hit **Claim Rewards** to collect:\n\n"
            "🪙 **200 Race Credits**\n"
            "🏁 **1 extra Career Match**\n"
            "📦 **1 Base Pack** (cards added to your collection)\n\n"
            "⏰ You can vote and claim once every **12 hours**."
        ),
        color=0xFF5F5F,
    )
    if bonus > 0:
        embed.add_field(
            name="🏁 Bonus Matches",
            value=f"You have **{bonus}** bonus career {'match' if bonus == 1 else 'matches'} saved! Use them with `/career_match`.",
            inline=False,
        )
    if can_claim:
        embed.add_field(name="✅ Ready to claim!", value="Vote on Top.gg then press **Claim Rewards** below.", inline=False)
    else:
        embed.add_field(name="⏳ Already claimed", value="Come back in 12 hours for your next vote reward.", inline=False)
    embed.set_footer(text="Every vote helps the bot grow — thank you! 🙏")

    view = VoteView(player_id, can_claim)
