...
def generate_turn_commentary(
    ...
    if lap == total_laps and (turn - 1) % 4 == 0 and turn > 1:
        lines.append(
            f"🔔 **FINAL LAP BEGINS!** Every tenth, every decision — it all matters NOW!"
        )
    ...
    if event:
        if "Rain" in event:
            lines.append(
                "🌧️ **RAIN! WEATHER CHANGE!** The circuit is getting wet — "
    ...
    if p1_pit and p2_pit:
        ...
        if gap > 0:
            ...
            lines.append(
                ...
                "fresh tyres could be the key to hunting down {p1_name}!*"
            )
    ...
    if weather == "rain":
        ...
        if wrong:
            lines.append(
                f"🌧️ *{' & '.join(wrong)} on DRY rubber in the WET — "
